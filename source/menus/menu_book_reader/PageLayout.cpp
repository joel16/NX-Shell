...
PageLayout::PageLayout(fz_document *doc, int current_page):doc(doc),pdf(pdf_specifics(ctx, doc)),pages_count(fz_count_pages(ctx, doc)) {
    ...
    SDL_RenderGetViewport(SDL_GetRenderer(SDL_GetWindow()), &viewport);
...
void PageLayout::draw_page() {
    ...
    SDL_RenderCopy(SDL_GetRenderer(SDL_GetWindow()), page_texture, NULL, &rect);
}

char *PageLayout::info() {
    char *title = static_cast<char*>(malloc(1024));
    snprintf(title, 1024, "%i/%i, %.2f%%", _current_page + 1, pages_count, zoom * 100);
    return title;
}
...
void PageLayout::render_page_to_texture(int num, bool reset_zoom) {
    ...
    page_texture = SDL_CreateTextureFromSurface(SDL_GetRenderer(SDL_GetWindow()), image);
